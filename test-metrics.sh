...
echo ""
echo "=================================================="
echo " STEP 5: GatewayProxyErrors"
echo " Threshold: 10 over 2x5min periods"
...
# evaluation_periods=2 so alarm needs two consecutive periods
...
# -------------------------------------------------------
# STEP 8 — AbandonedOrders (time delayed, cannot confirm
# alarm in this script run — prints a follow-up command)
# -------------------------------------------------------
echo ""
echo "=================================================="
echo " STEP 8: AbandonedOrders (35 min wait required)"
echo " Threshold: 10/hour  |  Placing: 15 orders to abandon"
echo "=================================================="

placed_abandoned=0
for i in {1..15}; do
  id=$(curl -sf -X POST "${ALB_URL}/api/orders/" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"items":[{"product_id":"test-product-001","quantity":1,"price":10.00}]}' \
    2>/dev/null | jq -r .id 2>/dev/null)
  [ -n "$id" ] && [ "$id" != "null" ] && placed_abandoned=$((placed_abandoned+1))
  printf "."
  sleep 0.3
done
echo ""
success "Placed ${placed_abandoned}/15 orders to abandon — do NOT advance these"
record "AbandonedOrders alarm" "skip"

echo ""
warn "Come back in 35 minutes and run this to confirm the alarm fired:"
echo ""
echo "  aws cloudwatch describe-alarms \\"
echo "    --alarm-names \"${ENVIRONMENT}-abandoned-orders-high\" \\"
echo "    --region ${REGION} \\"
echo "    --query 'MetricAlarms[0].StateValue' \\"
echo "    --output text"
echo ""
echo "  Expected output: ALARM"
echo ""
echo "  If it shows OK, check the scheduler logs:"
echo "  aws logs filter-log-events \\"
echo "    --log-group-name \"/ecs/${ENVIRONMENT}-scheduler\" \\"
echo "    --start-time \$(date -d \"-60 minutes\" +%s%3N 2>/dev/null || python3 -c \"import time; print(int((time.time()-3600)*1000))\") \\"
echo "    --filter-pattern \"abandoned\" \\"
echo "    --region ${REGION} \\"
echo "    --query 'events[*].message' --output text"
...
echo "  ${ENVIRONMENT}-abandoned-orders-high         check in 35 minutes"
echo ""
...
if [ "$TESTS_FAILED" -gt 0 ]; then
  ...
  echo "     Fix: the gateway test now sends two batches to cover both windows"
